...
def apply_kl_contrastive(
    ...
    kl_cmve_apply_mode="all",
):
    # not used in GRPO
    batch_size = data.batch.batch_size[0]
    response_mask = data.batch["response_mask"]

    if kl_cmve_apply_mode == "correct_only":
    ...
    kld_contrastive = core_algos.compute_kl(
        ...
        data.batch["cmve_log_probs"],
        kl_penalty=kl_penalty_contrastive,
    )
    kld_contrastive = kld_contrastive * response_mask
    data.batch["token_level_rewards"] += (
        kl_ctrl_contrastive.kl_coef * kld_contrastive
    )  # maximize
    ...
    metrics = {
        "kl_contrastive/kl": current_kl_contrastive,
        "kl_contrastive/kl_coef": kl_ctrl_contrastive.kl_coef,
    }
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # define KL schedular
        if config.worker.actor.model.apply_cmve:
            if config.algorithm.kl_cmve_schedule == "fixed":
                self.kl_ctrl_contrastive = core_algos.FixedKLController(
                    init_kl_coef=config.algorithm.kl_cmve_coef
                )
            elif config.algorithm.kl_cmve_schedule == "annealing":
                start_value = config.algorithm.kl_cmve_schedule_args.get(
                    "start_value", config.algorithm.kl_cmve_coef
                )
                end_value = config.algorithm.kl_cmve_schedule_args.get("end_value", 0.0)
                annealing_ratio = config.algorithm.kl_cmve_schedule_args.get(
                    "annealing_ratio", 0.5
                )
                annealing_steps = int(self.training_steps * annealing_ratio)
                print(
                    f"Using annealing KL schedule with start value {start_value}, end value {end_value}, and total steps {annealing_steps}."
                )
    ...
    def _get_kl_cmve_weights(
        self, batch: DataProto, reward_metrics: Dict[str, Any]
    ) -> DataProto:
        if self.config.algorithm.kl_cmve_apply_mode == "all":
            batch.non_tensor_batch["kl_cmve_weighting"] = np.array(
                [1.0] * len(batch.batch)
            )
        else:
            raise NotImplementedError(
                f"Unknown contrastive KL apply mode: {self.config.algorithm.kl_cmve_apply_mode}."
            )
        return batch

    def _get_kl_cmve_coef(self, batch: DataProto) -> DataProto:
        batch.non_tensor_batch["kl_cmve_coef"] = np.array(
            [self.kl_ctrl_contrastive.kl_coef] * len(batch.batch)
        )
        # update
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                if self.config.worker.actor.model.apply_cmve:
                    if reward_metrics is None:
                        reward_ref = self.reward_fn.compute_reward.remote(batch)
                        reward_tensor, reward_metrics = ray.get(reward_ref)

                    # store acc reward in batch
                    batch = self._get_kl_cmve_weights(batch, reward_metrics)
                    # store kl_cmve coef in batch
                    batch = self._get_kl_cmve_coef(batch)
                ...
                # compute cmve log_probs
                if self.config.worker.actor.model.apply_cmve:
                    # compute log_probs with disturbed context
                    with timer("cmve_probs", timing_raw):
                        cmve_log_probs = (
                            self.actor_rollout_ref_wg.compute_log_probs_cmve(batch)
                        )
